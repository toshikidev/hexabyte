...
summary: 本文介绍如何使用 Cloudflare Workers 快速搭建反向代理：给出可直接粘贴的核心脚本与关键参数（上游域名、路径、HTTPS、缓存开关、地区与 IP 封锁、文本替换规则等），并提供部署步骤（创建 Worker、粘贴代码、替换 example.com、保存与发布）。同时强调请求配额、合规性、性能调优与安全防护等注意事项，适合想要通过边缘网络为目标站点提供稳定加速与访问控制的场景。
